...
class frontController_v2 extends Controller
{
    ...
    public function searchArea()
    {
        ...
        foreach ($area as $areaData) {
            $temp = array(
                ...
                "url" => route('area',$areaData->area_url_en),
            );
            array_push($list,$temp);
            $temp = array(
    ...
    public function searchUniversity()
    {
        ...
        foreach ($university as $universityData) {
            $temp = array(
                ...
                "url" => route('university',$universityData->university_url_en),
            );
            array_push($list,$temp);
            $temp = array(
    ...
    public function searchLandmark()
    {
        ...
        foreach ($landmarkType as $landmarkTypeData) {
            ...
            $temp = array(
                ...
                "url" => route('landmark',[$landmarkTypeData->landmark_id,$landmarkTypeData->landmark_name_en]),
            );
            array_push($list,$temp);
        }
        return response()->json($list);
    ...
    public function news()
    {
        ...
        foreach ($news as $data) {
                $temp = array(
                    ...
                    'datetime' => date('d/m/Y',strtotime($data->news_datetime)),
                    'detail' => mb_substr(strip_tags(str_replace("&nbsp;"," ",$data->news_description_en)),0,200,"utf-8"),
    ...
    {
        // ต้องใส่เพื่อเปลี่ยนภาษา******************************************
        $url = env('URLTH').'/สลิปเงินเดือน'; $data['url'] = $url;
        // ต้องใส่เพื่อเปลี่ยนภาษา******************************************
        return view('front_v2.ourteam');
    }
