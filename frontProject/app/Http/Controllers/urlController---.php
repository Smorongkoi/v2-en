...
class urlController extends Controller
{
    public function getUrlArea()
    {
        ...
        $data['url'] = route('area',$area->area_url);
    ...
    public function getUrlTrain()
    {
        ...
        ->first();
        $data['url'] = route('station',$train->train_station_url);
        return response()->json($data);
    }
    public function getUrlUniversity()
    {
        ...
        ->first();
        $data['url'] = route('university',$university->university_url);
        return response()->json($data);
    }
    public function getUrlLandmark()
    {
        ...
        ->first();
        $data['url'] = route('landmark',[$landmarkType->landmark_id,$landmarkType->landmark_name]);
        return response()->json($data);
    }
    public function getUrlCondo()
    {
        ...
        ->first();
        $data['url'] = route('condo',[$condo->condo_url_th]);
        return response()->json($data);
    }
    public function getUrlPost()
    {
        ...
        $post = Post::find($id);
        if ($type == 1) {
            $post_url = route('rent',[$post->id,$post->post_no,str_replace([' ','/'],'-',$post->post_header)]);
        } else if ($type == 2) {
            $post_url = route('sale',[$post->id,$post->post_no,str_replace([' ','/'],'-',$post->post_header)]);
        }
